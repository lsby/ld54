...
节点移动' p a = case a.父节点信息 of
    ...
    Just (MK_父节点信息 f l r) =>
        let 父节点距离 = l
            ...
            角度范围起点 = 角度转弧度 $ fst r
            角度范围终点 = 角度转弧度 $ snd r
            角度范围起点虚拟位置 = 转换到参考系 (映射位置值 (\a => a * -1) f.位置) $极坐标转平面直角坐标 (MK_极坐标点 100 角度范围起点)
            角度范围终点虚拟位置 = 转换到参考系 (映射位置值 (\a => a * -1) f.位置) $极坐标转平面直角坐标 (MK_极坐标点 100 角度范围终点)
            离目标点近的边界 = if (计算两点间距离 p 角度范围起点虚拟位置) > (计算两点间距离 p 角度范围终点虚拟位置) then 角度范围终点 else 角度范围起点
            最终目标位置 = if 目标点极角 < 角度范围起点 then 转换到参考系 (映射位置值 (\a => a * -1) f.位置) $ 极坐标转平面直角坐标 $ {极角 := 离目标点近的边界} 目标点极坐标
                           else if 目标点极角 > 角度范围终点 then 转换到参考系 (映射位置值 (\a => a * -1) f.位置) $ 极坐标转平面直角坐标 $ {极角 := 离目标点近的边界 } 目标点极坐标
                           else p
